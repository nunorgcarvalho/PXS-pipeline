...
LMM_outs <- tibble(
  ...
  trait = str_replace(substring(LMM_files, 5), '.bgen.txt',''),
  h2_cohort = sapply(str_split(trait, "\\."), `[`, 1),
  training_cohort = sapply(str_split(trait, "-"), `[`, 2),
  model_factors = sapply(str_split(trait, "-"), `[`, 3),
  title = paste0('h2 cohort: ', h2_cohort, ' :: Training cohort: ', training_cohort,
                 ' :: Model factors: ', str_replace_all(model_factors,'_'," + "))
)
...
for (i in 1:nrow(LMM_outs)) {
  ...
  print(paste0(trait, " :: Loading full summary file"))
  ...
  print(paste0(trait, " :: Downscaling summary file"))
  ...
  print(paste0(trait, " :: Printing Manhattan plot"))
  gg<-ggplot(manhattan_tbl, aes(x = BP_cum, y = -log10(P_BOLT_LMM_INF))) +
    ...
    theme_light() +
    labs(title = trait, subtitle=subtitle) +
    xlab("Chromosome") +
  ...
  print(paste0(trait, " :: Saving Manhattan plot"))
  ...
  print(paste(trait, N, n_sig_SNPs, round(lambda,2)))
}

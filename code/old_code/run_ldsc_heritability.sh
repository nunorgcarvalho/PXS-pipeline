...
echo 'All exposures:' ${lines[@]} 'PXS_T2D'
for exposure in $lines 'PXS_T2D'; do
  ...
  # uses differently formatted path for PXS_T2D
  if [ "$exposure" == "PXS_T2D" ]; then
    dir_expo=${dir_scratch}T2D/
    loc_LMM_out=${dir_expo}T2D_PXS_BOLTLMM.out
  fi
...
# also runs analysis for PXS-T2D

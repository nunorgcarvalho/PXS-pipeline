...
ggplot(test, aes(x=`BRS-females-cov_bvr`,y=`BRS-males-cov_bvr`,
                  color = sex)) +
  geom_point(alpha=0.1, shape=1) +
  geom_abline(slope=1, color=dash_color, linetype='dashed') +
  geom_smooth(method='lm', formula='y ~ x', se=FALSE) +
  annotate('text', label = annotate_cor.test(cor_sex), # throws out harmless warning message
...
ggplot(test, aes(x=`BRS-T2D_early-cov_bvr`,y=`BRS-T2D_late-cov_bvr`,
                 color = T2D_onset_cat)) +
  geom_point(alpha=0.1, shape=1) +
  geom_abline(slope=1, color=dash_color, linetype='dashed') +
  geom_smooth(method='lm', formula='y ~ x', se=FALSE) +
  annotate('text', label = annotate_cor.test(cor_time), # throws out harmless warning message
           x=Inf, y=-Inf, vjust=-0.5, hjust=1.05, parse=FALSE) +
...
ggplot(test, aes(x=`BRS-ALL-cov_bvr`,y=`BRS-ALL-cov_SES`,
                 color = SES)) +
  geom_point(alpha=0.1, shape=1) +
  geom_abline(slope=1, color=dash_color, linetype='dashed') +
  geom_smooth(method='lm', formula='y ~ x', se=FALSE) +
  annotate('text', label = annotate_cor.test(cor_SES), # throws out harmless warning message
           x=Inf, y=-Inf, vjust=-0.5, hjust=1.05, parse=FALSE) +
...
ggplot(SES_lm, aes(x=a,y=b, color=SES)) +
  geom_point() +
  geom_text(aes(label = SES, y=b_upp+0.001)) +
...
ggplot(SES_models, aes(x=fct_rev(factor(model_label, levels=model_label)), y=AUC)) +
  geom_point() +
...
## corr(SES, BRS) ####
cor.test(pheno_SES$f738, pheno_SES$cov_bvr)
ggplot(pheno, aes(x=SES, y=`BRS-ALL-cov_bvr`)) +
  geom_boxplot() +

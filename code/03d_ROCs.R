...
ROC_tbl <- tibble(term = c('cov',col_bvrs,'BRS',col_CRFs,'CRS'))
...
for (i in 1:nrow(ROC_tbl)) {
  ...
  if (term == 'cov') {
    ...
    cols_use <- c(col_covs, col_bvrs_ALL)
  } else if (term == 'CRS') {
    cols_use <- c(col_covs, col_CRFs[-4]) # excludes CRF
  } else {
...
save(cvglm_list, file='scratch/BRS_models/cvglm_list_terms.RData')
save(ROC_list, file='scratch/BRS_models/ROC_list_terms.RData')
...
# saves to system
fwrite(ROC_tbl, file='scratch/general_results/ROC_tbl.tsv')

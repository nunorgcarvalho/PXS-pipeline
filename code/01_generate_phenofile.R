# generates phenotype file containing T2D outcomes, covariates, behaviors, and 
...
loc_expos <- paste0(dir_script,"../input_data/UKBB_exposures.txt")
...
colnames(pheno_joint)[colnames(pheno_joint) %in% CRFs_tbl$field_raw] <- sapply(colnames(pheno_joint)[colnames(pheno_joint) %in% CRFs_tbl$field_raw],
...
fields <- tibble(term = c(col_covs,colnames(data_PHESANT[-1]))) %>%
  ...
  mutate(use_type = ifelse(field %in% col_covs, "covar",
                           ifelse(field %in% CRFs_tbl$fieldID, "CRF","behavior")),
...
# makes pheno table that will be saved ####
pheno <- T2D_tbl %>%
...
loc_out <- paste0(dir_scratch, "pheno_fullT2D.txt")

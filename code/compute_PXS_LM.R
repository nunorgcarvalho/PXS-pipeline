...
exposures_tbl <- fields %>% full_join(coeffs, by=c("code"="X")) %>%
...
num_cols_skip <- ncol(pheno_wide) - nrow(exposures_tbl)
PXSs <- c()
...
PXS_lm_tbl <- pheno %>% select(FID,IID,all_of(covars)) %>%
  left_join(out_PXS, by=c("FID","IID")) %>%
  rename("sex" = "f.31.0.0",
         "age" = "f.34.0.0",
